@extends("temasBlade.app")

@section('content')
    <div class="flex justify-center flex-wrap bg-gray-200 p-4 mt-5">
        <div class="text-center">
        <h1 class="mb-5">{{ __("Lista de Productos")}}</h1>
        <a href="{{route("articles.create")}}" class="bg-transparent hover:bg-green-500 text-green-700 font-semibold hover:text-white py-2 px-4 border border-green-500 hover:border-transparent rounded">
        {{ __("Añadir Productos")}}
        </a>
        </div>
    </div>
            ...
            @forelse($articles as $article)
                <tr>
                    ...
                    <td class="border px-4 py-2">
                        <a href="{{ route("articles.edit", ["article" => $article]) }}" class="text-green-400">{{ __("Editar") }}</a> |
                        <a
                            href="#"
                            class="text-red-400"
